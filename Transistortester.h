...
// Big Capacities (>50ÂµF) are measured with up to 500 load-pulses with the 680 Ohm resistor.
...
//generate Omega- and Âµ-character as Custom-character, if these characters has a number of loadable type
#if LCD_CHAR_OMEGA < 8
   const unsigned char CyrillicOmegaIcon[] MEM_TEXT = {0,0,14,17,17,10,27,0};	//Omega
#endif
#if LCD_CHAR_U < 8
   const unsigned char CyrillicMuIcon[] MEM_TEXT = {0,17,17,17,19,29,16,16};	//Âµ
#endif
...
#endif
#if LCD_CHAR_U < 8
  extern const unsigned char CyrillicMuIcon[] MEM_TEXT;	//Âµ
#endif
